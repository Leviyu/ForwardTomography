...
int main()
{
	...
	my_tomo.MODEL_model = model_path+"/"+MODEL+".model";
	my_tomo.MODEL_INFILE = model_path+ "/"+MODEL+".INFILE";
	my_tomo.read_INFILE();
	my_tomo.read_MODEL_INFILE();
	my_tomo.flag_is_new_tomo = "new";
	my_tomo.read_tomography();

	new_tomo tomo2;
	tomo2.convert_to_new_tomo(&my_tomo);
	...
	//my_big_record.read_record_file(&my_big_record.my_record[0]);
	...
	tomo2.output_tomography_info3("final");
	tomo2.output_tomography_info2(".standard.model");
