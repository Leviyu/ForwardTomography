...
void new_record::read_cross_point_info(new_tomo* my_tomo)
{
	...
	this->cross_point_file = my_tomo->cross_point_dir + "/"+this->EQ+ "/cross_point."
		+ this->STA+ "."+this->PHASE+"."+this->COMP;
	...
	//while(!myfile.eof())
	{
		...
		//cout << " dep "<< this->CP_dep[count]
			//<< " dl "<< this->CP_dl[count]
			//<< " v_PREM " << this->CP_v_PREM[count]
			//<< " dt "<< this->CP_dl[count] / this->CP_v_PREM[count]
			//<< endl;
		//cout << " CP_ilat is "<< this->CP_ilat[count] << endl;
		...
		*/
		if( this->CP_dep[count] > this->bottoming_depth )
			this->bottoming_depth = this->CP_dep[count];
	}
	...
	{
		//string command;
		//command = "get_taup_time_3 "+ this->PHASE+" "+ std::to_string(this->eq_lat) + " "+
			//std::to_string(this->eq_lon) + " " +
			//std::to_string(this->eq_dep) + " " +
			//std::to_string(this->sta_lat) + " " +
			//std::to_string(this->sta_lon);
		//double taup_time = atof(exec(command).c_str());
		//double PREM_file = this->PREM;
		cout << " EQ_STA_PHASE "<< 
			this->EQ << " " <<
			this->STA << " "<<
			this->PHASE << " "<<
			" Code PREM time is "<< code_PREM_time<< endl;
			//" taup-PREM_FILE "<< taup_time - PREM_file << endl;
			//" Code PREM time is "<< code_PREM_time<< " diff "<< taup_time- code_PREM_time <<
			//" taup-PREM_FILE "<< taup_time - PREM_file << endl;
	}

	myfile.close();
	this->cross_point_file = 1;
...
void new_record::find_cross_points(new_tomo* my_tomo)
{
	...
	this->CP_num = this->taup_path_max_num -1;
	...
	for(count = 0; count < this->taup_path_max_num ; count ++)
	{
		...
		this->CP_idep[count]= my_tomo->find_index_dep2(this->CP_dep[count]);
		this->CP_ilat[count] = my_tomo->find_index_lat(this->CP_lat[count]);
		this->CP_ilon[count] = my_tomo->find_index_lon(this->CP_lon[count], this->CP_ilat[count]);
//cout << " dep/lat/lon "<< this->CP_dep[count] << "/"<< this->

		//cout << " find cross point "<< this->CP_idep[count] << " "<<this->CP_ilat[count] << " "<< this->CP_ilon[count] << endl;
		
		//cout << "count "<< count << " angle is "<< this->angle[count] << " lat lon is "<< lat_tmp << " lon " << lon_tmp << endl;
		//cout <<"index:"<<  index_dep << "       " << index_lat << "   " << index_lon << endl;
	...
	for(count = 0; count < this->CP_num  ; count ++)
	{
		...
		delta_angle = (this->angle[count +1]- this->angle[count] ) / 180 * M_PI;
		R1 = this->radius[count];
		R2 = this->radius[count+1];
		this->CP_dl[count] = sqrt( R1*R1 + R2*R2 - 2*R1*R2*cos(delta_angle) );
		//max_dl+=this->CP_dl[count];
		...
		//cout << " cout"<< count << " dl "<< this->CP_dl[count]<< endl;

		dep_haha =  ( this->CP_dep[count] + this->CP_dep[count+1] ) /2; 
		if(dep_haha > 2891 )
		{
			cout << " ERROR depth > 2891 depth haha is " <<dep_haha <<  endl;
			dep_haha = 2891;
		}
		this->CP_dep[count] = dep_haha;

		// find bottoming depth
		if( this->CP_dep[count] > this->bottoming_depth)
		{
			this->bottoming_depth = this->CP_dep[count];
		}
		//cout << " find bot depth "<< endl;

		prem_s(dep_haha, &PREM_tmp, 0 );
		this->CP_v_PREM[count] = PREM_tmp;

		// construct the weight to distribute travel time  residual
		my_tomo->weight_ratio_path_length_RMS_tomo = 1;
...
//cout << "max dl is "<< max_dl << endl;
	...
	for(count = 0; count < this->CP_num  ; count++)
	{
		this->CP_weight[count] = weight_cell[count] / SUM_weight *2000;
	}
	...
	for(count = 0; count < this->CP_num; count++)
	{
		if( this->CP_dl[count] == 0 )
		{
			cout << " ERROR CP_dl is zero "<< endl;
			continue;
		}
		myfile << fixed
			<< setw(15) << setprecision(8) << this->CP_dep[count]
			<< setw(15) << setprecision(8) << this->CP_lat[count]
			<< setw(15) << setprecision(8) << this->CP_lon[count]
			<< setw(15) << setprecision(8) << this->CP_idep[count]
			<< setw(15) << setprecision(8) << this->CP_ilat[count]
			<< setw(15) << setprecision(8) << this->CP_ilon[count]
			<< setw(15) << setprecision(8) << this->CP_dl[count]
			<< setw(15) << setprecision(8) << this->CP_v_PREM[count]
			<< setw(15) << setprecision(8) << this->CP_weight[count]
			<< endl;
		//cout << " cout "<< count << " weight "<< this->CP_weight[count]<<
			//" CP dep"<< this->CP_dep[count]<<endl;
...
void new_record::read_taup_path_info(string taup_path_dir)
{
	...
	this->taup_path_file = taup_path_dir +"/"+ this->EQ+"/taup_path."+this->STA+"."+this->PHASE;
	...
	for(count = 0 ; count < line_max;count++)
	{
		...
		myfile >> this->CP_lat[count];
		myfile >> this->CP_lon[count];
		this->CP_dep[count] = 6371.0-this->radius[count];
		//cout << " angle "<< this->angle[count] << "   "<< this->radius[count] << endl;
		
		//old_angle = angle[count];
	}

	this->taup_path_max_num = line_max;
	//cout << " taup path max points is "<< line_max<<endl;
	 //define new array
	//int new_npts;
	//double dt;
...
void new_record::initiate_CP(int line_max)
{
	//cout << "--> Initiate CP info " << endl;
	int MAX1 = line_max;
	this->CP_v_PREM.resize(MAX1);
	this->CP_lat.resize(MAX1);
	this->CP_lon.resize(MAX1);
	this->CP_dep.resize(MAX1);
	this->CP_ilat.resize(MAX1);
	this->CP_idep.resize(MAX1);
	this->CP_ilon.resize(MAX1);
	this->CP_dl.resize(MAX1);
	this->CP_weight.resize(MAX1);
	this->current_CP_weight.resize(MAX1);
	//this->angle.resize(MAX1);
	//this->radius.resize(MAX1);
...
void new_record::free_CP()
{
	...
	this->CP_idep.clear();
	this->CP_dl.clear();
	this->CP_weight.clear();
	this->current_CP_weight.clear();
...
void new_record::calculate_tomo_correction(new_tomo* my_tomo)
{
	...
	//cout << " i dep/lat /lon " << idep << " "<< ilat << " "<< ilon << " "<< dvs_tomo << endl;
		velocity_tmp = this->CP_v_PREM[count] * (1+ dvs_tomo / 100);

		//if(velocity_tmp == 0)
			//cout << " ERROR velocity is zero "<< endl;

		this->dt_tomo_correction += (this->CP_dl[count] /  velocity_tmp ) 
			- (this->CP_dl[count] / this->CP_v_PREM[count]);

		if( this->CP_v_PREM[count] == 0)
			cout << "ERROR PREM is zero idep ilat ilon  " << idep << "   " << ilat << "   " << ilon << endl;


	if( this->dt_tomo_correction != this->dt_tomo_correction)
	{
	cout << "dl "<< this->CP_dl[count]
		<< " velocity "<< velocity_tmp
		<< " CP_v_PREM "<< this->CP_v_PREM[count]
		<< " dvs tomo "<< dvs_tomo 
		<< endl;
	//cout << this->EQ << endl;
	}
	...
	if( fabs(this->dt_tomo_correction) >20 )
		cout << " ERROR dt_tomo_correction is greater then 20sec, something wrong! "<< endl;
	if( this->dt_tomo_correction != this->dt_tomo_correction)
		cout << "ERROR dt_tomo_correction is nan" << endl;
}
