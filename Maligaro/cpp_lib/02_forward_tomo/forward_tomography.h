...
class new_cell
{
	...
	double 	sum_num_in_cell;
	double 	sum_num_in_cell_weight;
...
class new_tomo
{
	public:
		...
		new_tomo(const new_tomo& rhs){};
		new_tomo* operator=(const new_tomo& rhs){};
		...
		vector< vector< vector< int > > > smoother;
		...
		int output_tomography_info2(string name);
		int output_tomography_info3(string name);
		int output_starting_tomography();

		int check_if_update_with_current_record(new_record* my_record);

		int convert_to_new_tomo(new_tomo* old_tomo);
		int define_vertical_profile();
		int define_horizontal_profile();
		int define_horizontal_profile_orig();
		int store_smoother(big_new_record* my_big_record);
		int smooth_dvs_model_for_each_cell(big_new_record* my_big_record,
			int idep2,int ilat2,int ilon2);
		double current_sig; // sig for gaussian
		int current_index;
		int get_sig_index();
		int update_tomo_for_current_iteration_add_delta_dvs();
		...
		double get_pair_weight(int idep, new_record* my_record);
		...
		vector<double> layer_weight;
		vector<double> dt_path_len_weight;
		double get_delta_lat_lon_distance_with_depth(int index_dep);

		vector<double> L2_weight;
		int construct_L2_weight();
		vector<double> L1_weight;
		int construct_L1_weight();


		void construct_RMS_profile(string flag);
		...
		int free_CP_memory_for_records(big_new_record* my_big_record);
		
		int update_step_flag;		
		// 1 ~ step 1
		// 2 ~ step 2
		// 3 ~ step 3
		...
		void update_tomo_for_current_iteration_clean_num_in_cell();
		...
		void output_delta_tomography(new_tomo* my_tomo,string name);
		...
		int S1_update(new_record* my_record);
		int calculate_variance_for_current_update(big_new_record* my_big_record);
		int make_absolute_checkerboard();
		...
		int calculate_turning_depth(new_record* my_record);
		...
		int output_tomography_info_standard();
		int read_standard_tomo();

		// updating parameter
		int update_phase_num;
		vector<string> update_phase_list;
		int bottom_layer;
		int update_layer;
...
//
		...
		void read_record_file(vector<new_record>& my_record);
		void initiate_big_record();
		void read_INFILE();
		double get_phase_dep_weight(string PHASE);
