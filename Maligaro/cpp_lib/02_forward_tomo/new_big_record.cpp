...
void big_new_record::read_record_file(vector<new_record>& my_record)
{
	...
	{
		getline(myfile,tmp);
		//cout << "read record "<< line+1 <<"/"<< this->sta_num<< endl;
		//continue;
		//cout << tmp << endl;
		istringstream ss(tmp);
		...
		for(count = 1 ; count <= 47 ; count ++)
		{
			...
			else if( count == 30 )
			{
				...
				if( my_record[line].record_weight == 0)
					my_record[line].record_weight = 0.2;
			}
			else if( count == 31 )
			...
			else if( count == 45 )
			{
				...
				my_record[line].pair1 = atof(sub1.c_str());
			else if( count == 47 )
				my_record[line].pair1dep = atof(sub1.c_str());


			// define phase dependent weight
			my_record[line].phase_dep_weight = this->get_phase_dep_weight(my_record[line].PHASE);
			//cout << " current phase weight is "<< my_record[line].phase_dep_weight << endl;


		}

	//cout << my_record[line].EQ << " "<< my_record[line].STA << endl;

			//cout << " pair "<< my_record[line].pair1  << " "<< my_record[line].pair1dep << endl;
	...
	cout << " <---- read record file done" << endl;

}
double big_new_record::get_phase_dep_weight(string PHASE)
{

	if( PHASE.compare("S") == 0)
		return 1.0;
	else if( PHASE.compare("SS") == 0)
		return 0.8;
	else if( PHASE.compare("SSS") == 0)
		return 0.7;
	else if( PHASE.compare("ScSScS") == 0)
		return 0.65;
	else if( PHASE.compare("ScS") == 0)
		return 0.70;
	else if( PHASE.compare("Sdiff") == 0)
		return 1.0;
	else
		return 0.5;
