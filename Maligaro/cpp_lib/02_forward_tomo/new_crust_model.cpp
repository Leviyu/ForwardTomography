...
{
	cout << " --> Crust Model is defined "<< endl;

	int CRUST_LINE_MAX = 9;
	int global_npts = 70000;

	this->boundary.resize(CRUST_LINE_MAX);
	this->vs.resize(CRUST_LINE_MAX);  
	this->vp.resize(CRUST_LINE_MAX);  
	this->rho.resize(CRUST_LINE_MAX);  
	int count;
	for(count = 0; count < CRUST_LINE_MAX;count++)
	{
		this->boundary[count].resize(global_npts);  
		this->vs[count].resize(global_npts); 
		this->vp[count].resize(global_npts); 
		this->rho[count].resize(global_npts); 
	}
}
...
cout << "---> Read in CRUST 1.0 model" << endl;
	...
	// read in crust vs
	current_file_name = "./CRUST10/crust1.vs";
	myfile.open(current_file_name.c_str());
	for( i = 0; i < 180*360; i++)
	{
		for(layer_num = 0; layer_num < CRUST_LINE_MAX; layer_num++)
			myfile >> this->vs[layer_num][i];
	}
	myfile.close();

	// read in crust vp
	current_file_name = "./CRUST10/crust1.vp";
	myfile.open(current_file_name.c_str());
	for( i = 0; i < 180*360; i++)
	{
		for(layer_num = 0; layer_num < CRUST_LINE_MAX; layer_num++)
			myfile >> this->vp[layer_num][i];
	}
	myfile.close();


	// read in crust boundry
	current_file_name = "./CRUST10/crust1.bnds";
	myfile.open(current_file_name.c_str());
	for( i = 0; i < 180*360; i++)
	{
		for(layer_num = 0; layer_num < CRUST_LINE_MAX; layer_num++)
		{
			myfile >> this->boundary[layer_num][i];
			//cout << " boundary "<< this->boundary[layer_num][i] << endl;
		}
...
//cout << "index lat lon is current index"<< index_lat << " " << index_lon << " "<< current_index << endl;
	...
	for(count = 0; count < CRUST_LINE_MAX ;count ++)
	{
		...
		//double vvvs;
		//prem_s(-1*bd[count],&vvvs,0);
		//cout << " lauer "<< count << "bd "<<-1* bd[count] << " vs "<< vs[count] << " prem "<< vvvs << endl;
	}
